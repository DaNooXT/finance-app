...
from filter.movimentation import MovimentationFilter, normalize_movimentation_filters
...
async def list_movimentation (request: Request, current_user: User = Depends(get_current_user), session: Session = Depends(get_db)):
    filters = normalize_movimentation_filters(dict(request.query_params))
    service = MovimentationServices(current_user, session)
    return service.show_movimentations(filters, current_user)
